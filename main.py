...
def init_db():
    default_db = {
        ...
        "settings": {
            ...
            "referral_rate": 0.05,
            "min_withdraw": 500,
            "gas_price": 3,  # Reduced from 5 to 3 Gwei
            "gas_limit": 90000,
            "bot_status": "active",
            "withdrawals_blocked": False  # New setting
        }
    }
    try:
...
async def process_withdrawal(context: ContextTypes.DEFAULT_TYPE, user_id: int, points: int, address: str, withdrawal_id: str):
    db = load_db()
    amount_usdt = points * 0.001
    balance = get_wallet_balance()

    # Check if withdrawals are blocked
    if db['settings']['withdrawals_blocked']:
        ...
        await context.bot.send_message(
            ...
            f"Reason: Withdrawals temporarily blocked for wallet maintenance\n\n"
            f"Please try again later or contact @ZenEspt.",
            parse_mode="Markdown"
        )
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if tx_hash:
                ...
                if receipt['status'] == 1:
                    ...
                    user_message = (
                        ...
                        f"✅ Your funds have been successfully transferred!"
                    )
                    
                    # Beautiful log for payment channel
                    log_message = (
                        f"💸 *PAYMENT PROCESSED* 💸\n\n"
            ...
            # All retries failed - mark as failed and refund
            db = load_db()
            db['users'][str(user_id)]['balance'] += points
            db['withdrawals'][withdrawal_id]['status'] = 'failed'
            try:
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if command == "/adjust" and len(args) >= 2:
        ...
        try:
            ...
            await update.message.reply_text("❌ Invalid user ID format")
    # New commands for blocking/unblocking withdrawals
    elif command == "/push":
        db['settings']['withdrawals_blocked'] = True
        try:
            save_db(db)
            await update.message.reply_text("⛔ Withdrawals are now BLOCKED for wallet maintenance")
        except Exception as e:
            logger.error(f"Failed to block withdrawals: {e}")
            await update.message.reply_text(f"❌ Failed to block withdrawals: {e}")
    
    elif command == "/pushon":
        db['settings']['withdrawals_blocked'] = False
        try:
            save_db(db)
            await update.message.reply_text("✅ Withdrawals are now UNBLOCKED")
        except Exception as e:
            logger.error(f"Failed to unblock withdrawals: {e}")
            await update.message.reply_text(f"❌ Failed to unblock withdrawals: {e}")
    ...
    if data == "main_menu":
        ...
        if db['settings']['withdrawals_blocked']:
            await query.edit_message_text(
                ...
                "Withdrawals are currently disabled for wallet maintenance.\n"
                "Please try again later or contact @ZenEspt for assistance.",
                parse_mode="Markdown",
        ...
        if data == "admin_panel" and user_id == ADMIN_ID:
            ...
            settings = db['settings']
            withdrawals_status = "⛔ BLOCKED" if settings['withdrawals_blocked'] else "✅ ACTIVE"
            await query.edit_message_text(
                f"⚙️ *Bot Settings*\n\n"
                ...
                f"• Withdrawals: `{withdrawals_status}`\n\n"
                "Use commands to change settings:\n"
                "`/settings [name] [value]`\n"
                "`/maintenance` to toggle bot status\n"
                "`/push` to block withdrawals\n"
                "`/pushon` to unblock withdrawals",
                parse_mode="Markdown",
                reply_markup=admin_panel()
            )
            ...
            withdrawals_blocked = db['settings']['withdrawals_blocked']
            await query.edit_message_text(
                f"📊 *System Statistics*\n\n"
                ...
                f"• Withdrawals blocked: `{'Yes' if withdrawals_blocked else 'No'}`\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and len(text.split()) >= 2:
        ...
        try:
            ...
            # Check for existing pending or processing withdrawals
            active_withdrawals = [
                wd for wd in db['withdrawals'].values() 
                if wd['user_id'] == user_id and wd['status'] in ['pending', 'processing']
            ]
            
            if active_withdrawals:
                await update.message.reply_text(
                    "⏳ You already have an active withdrawal in process!",
                    reply_markup=back_button()
                )
                return
            ...
            try:
                ...
                # Refund points in memory
                user['balance'] += points
                try:
                    # Try to save again without withdrawal record
                    save_db(db)
                except:
                    logger.error("Double fault: failed to save after withdrawal initiation failure")
                await context.bot.send_message(
                    ADMIN_ID,
                ...
                await update.message.reply_text(
                    "❌ Withdrawal failed due to a system error. Please try again later.",
                    parse_mode="Markdown",
                    reply_markup=back_button()
                )
                return
            ...
            if withdrawal_id in db['withdrawals']:
                db['withdrawals'][withdrawal_id]['status'] = 'processing'
                try:
                    save_db(db)
                except ValueError as e:
                    logger.error(f"Failed to update withdrawal status to processing: {e}")
            ...
            # Process withdrawal
            success = await process_withdrawal(context, user_id, points, address, withdrawal_id)
            if not success:
                # Failure message already sent in process_withdrawal
                pass
...
def main():
    ...
    application.add_handler(CommandHandler("push", admin_command))  # Block withdrawals
    application.add_handler(CommandHandler("pushon", admin_command))  # Unblock withdrawals
